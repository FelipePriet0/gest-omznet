...
-- 1) Enum e colunas para decisão final do card
do $$
begin
  if not exists (select 1 from pg_type where typname = 'kanban_decision_status') then
    create type public.kanban_decision_status as enum ('aprovado', 'negado');
  end if;
end $$;

alter table public.kanban_cards
  add column if not exists decision_status public.kanban_decision_status,
  add column if not exists decision_at timestamptz,
  add column if not exists decision_by uuid references public.profiles(id);

create index if not exists idx_kanban_cards_decision_status on public.kanban_cards(decision_status);
create index if not exists idx_kanban_cards_decision_at on public.kanban_cards(decision_at);

-- 2) RPC para atualizar a decisão final do card
...
-- 3) Atualiza a função add_parecer para aceitar decisão por nota
...
-- 4) Atualiza função de edição de parecer para suportar decisão
