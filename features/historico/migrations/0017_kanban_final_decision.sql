-- Add final decision fields and enhance change_stage to capture decision at finalização

do $$ begin
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='kanban_cards' and column_name='final_decision'
  ) then
    alter table public.kanban_cards add column final_decision text;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='kanban_cards' and column_name='finalized_by'
  ) then
    alter table public.kanban_cards add column finalized_by uuid references public.profiles(id);
  end if;
exception when others then null; end $$;

-- Override change_stage to store final_decision and finalized_by when entering 'finalizados'
do $$ begin
  ...
  begin
    ...
    if p_area = 'analise' and p_stage = 'finalizados' then
      update public.kanban_cards kc
        set area = 'analise',
            stage = 'finalizados',
            final_decision = v_old_stage, -- capture the stage we came from (e.g., aprovados/negados)
